  ...
  cols.forEach((col) => {
    ...
    col.forEach((sec) => {
      ...
      if (sec.sizes && sec.sizes.length > 1) {
        ...
        sh.style.gridTemplateColumns = "36px minmax(0,1fr) repeat(" + sec.sizes.length + ", 58px)";
      ...
      (sec.items || []).filter((it) => it && it.visible !== false).forEach((it) => {
        ...
        el.style.gridTemplateColumns = nS > 1 ? ("36px minmax(0,1fr) repeat(" + nS + ", 58px)") : "36px minmax(0,1fr) 52px";
